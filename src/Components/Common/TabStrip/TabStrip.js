...
const TabStrip = (props) => {
    const content =props.tabs.map(({ title, location, count, notify }, key) => {
        return (
            <NavLink to={props.match.url + "/" + location} activeClassName="border-nebula-blue" key={key} className={"flex border-b-2 border-transparent pb-3 pt-3 px-4 font-semibold hover:bg-nebula-blue-light transition duration-300 "}>
                <div
                    key={location}
